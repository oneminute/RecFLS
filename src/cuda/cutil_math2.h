...
namespace cuda
{
    ...
    inline __device__ void dsyevc2(float A, float B, float C, float* rt1, float* rt2) {
        float sm = A + C;
        float df = A - C;
    ...
    // ----------------------------------------------------------------------------
    inline __device__ void dsyev2(float A, float B, float C, float* rt1, float* rt2, float* cs, float* sn)
    {
        float sm = A + C;
        float df = A - C;
        float rt = sqrt(SQR(df) + 4.0 * B * B);
    ...
    //  -1: Error
    // ----------------------------------------------------------------------------
    inline __device__ int dsyevc3(float A[3][3], float w[3])
    ...
    // ----------------------------------------------------------------------------
    inline __device__ int dsyevv3(float A[3][3], float Q[3][3], float w[3])
        // ----------------------------------------------------------------------------
  ...
  // Calculate eigenvalues
    ...
    // ----------------------------------------------------------------------------
    inline __device__ void dsytrd3(float A[3][3], float Q[3][3], float d[3], float e[2])
        // ----------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------
    inline __device__ int dsyevq3(float A[3][3], float Q[3][3], float w[3])
        // ----------------------------------------------------------------------------
        // Calculates the eigenvalues and normalized eigenvectors of a symmetric 3x3
    ...
    inline __device__ void eig2(const float* M, float* V, float* L) {
    ...
    inline __device__ void eig3(const float* M, float* V, float* L, bool useIterative = false) {
    ...
    inline __global__ void eig(const float* M, float* V, float* L, const int n, bool useIterative = false) {
        if (n == 2) {
            eig2(M, V, L);
        }
        else if (n == 3) {
    ...
    inline __global__ void eigVal(const float* M, float* L, const int n) {
        if (n == 2) {
